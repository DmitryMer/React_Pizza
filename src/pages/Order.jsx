...
const Order = ({ orders, setOrders, totalPrice }) => {
    ...
    const orderPlaced = () => {
        if (orders.length > 0) {
            ...
            setModalOrderText(`Ваш заказ на сумму: ${totalPrice} $ оформлен! Спасибо за ваш заказ`)
        } else {
            setModalActive(false)
    ...
    return (
        <>
            <Modal text={modalOrderText} modalActive={modalActive} setModalActive={setModalActive} />
            <div
                style={{
                    display: "flex",
                    flexWrap: "wrap",
